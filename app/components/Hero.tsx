...
export default function Hero() {
  ...
  const SHIELDS = [T.trust1, T.trust2, T.trust3, "Win / macOS / Linux"];
            ...
            <div className="relative card-base rounded-2xl overflow-hidden">
              ...
              <div className="p-5 grid grid-cols-5 gap-4 items-stretch">
                ...
                <div className="col-span-2 flex flex-col gap-3">
                  <div className="card-base rounded-xl p-3.5 bg-pico-900 flex-1">
                    ...
                    <div className="font-display font-bold text-xl text-pico-text leading-none">63–74%</div>
                    <div className="text-2xs text-pico-muted mt-1">per 10-min window</div>
                    <div className="mt-2.5 h-1 rounded-full bg-pico-700 overflow-hidden">
                      <div className="h-full rounded-full bg-gradient-to-r from-pico-primary to-pico-primary/60" style={{ width: "68%" }} />
                    </div>
                  </div>
                  <div className="card-base rounded-xl p-3.5 bg-pico-900 flex-1">
                    <div className="text-2xs text-pico-muted mb-1">Mode</div>
                    <div className="font-display font-bold text-base text-pico-text leading-tight">Kbd + Mouse</div>
              ...
              <div className="mx-5 mb-5 rounded-xl border border-pico-700/60 bg-pico-950/80 overflow-hidden">
                <div className="flex items-center justify-between px-3.5 py-2 border-b border-pico-700/40">
                  <span className="text-2xs font-mono text-pico-subtle uppercase tracking-widest">activity log</span>
                ...
                <div className="p-3 space-y-1.5 font-mono text-xs">
                  {LOG_LINES.map((line, i) => (
                    <div key={i} className="flex items-start gap-2">
                      <span className="text-pico-primary shrink-0">[{line.time}]</span>
                      <span className="text-pico-muted">{line.text}</span>
                      {i === LOG_LINES.length - 1 && (
                        <span className="inline-block w-1.5 h-3.5 bg-pico-primary animate-blink ml-0.5 rounded-sm shrink-0" aria-hidden="true" />
                      )}
                    </div>
                  ))}
                </div>
